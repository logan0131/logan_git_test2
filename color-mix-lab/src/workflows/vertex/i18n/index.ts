...
const en: Dict = {
  ...
  progressLoadObj: "Parse model colours (OBJ / Rodin 3MF)",
  ...
  // --- Rodin pipeline: Feature A (Rodin 3MF) ---
  rodin3mfFile: "Rodin 3D Print 3MF (Face Color)",
  tipRodin3mfFile:
    "Loads the 3MF from the Hyper3D Rodin 3D Print tab (Output Mode: Face Color). Face colours come from the Bambu paint codes and the filament palette in project_settings.config; Texture Baking is skipped.",
  loadingRodin: "Loading Rodin 3MF...",
  rodinLoaded: "Rodin 3MF loaded:",
  rodinSummaryTitle: "Rodin 3MF",
  rodinPalette: "Palette",
  rodinUsedColours: "used",
  rodinUnpainted: "unpainted triangles",
  rodinPaletteFallback: "placeholder palette (no project_settings.config)",
  rodinTextureModeHint: "Texture Map exports are rejected: choose Output Mode = Face Color in Rodin.",

  // --- Feature B (merge with shade re-judgement) ---
  mergeReprojectToggle: "Re-judge shade colours + clean up",
  tipMergeReprojectToggle:
    "When enabled, Merge selected bakes the merge into the model: colours flagged as shade/boundary follow the surface they are connected to, boundaries are smoothed and small islands are absorbed. When disabled, the merge is only a virtual-extruder override (old behaviour).",
  mergeReprojectionTitle: "Shade re-judgement and cleanup",
  mergeReprojectionIntro:
    "Flags per palette colour. Shade/boundary colours are cleared during a merge and re-judged from their neighbours; protected colours are never touched by the cleanup. Defaults are suggested automatically (low saturation + mid value, or < 3% area; protect: < 1% area and saturated).",
  tipMergeReprojection:
    "Adjust which colours count as shade/boundary, which are protected, and the minimum island size per colour before pressing Merge selected.",
  mergeFlagArea: "Area",
  mergeFlagAmbiguous: "Shade / boundary",
  tipMergeFlagAmbiguous:
    "Treat this palette colour as a shadow or anti-aliased boundary colour. When it is part of a merge, its faces follow the majority of the connected certain faces instead of the user's merge target.",
  mergeFlagProtect: "Protect",
  tipMergeFlagProtect:
    "Never change faces of this colour during boundary smoothing or island absorption (small important accents such as red gems).",
  mergeFlagMinBlob: "Min. island (faces)",
  tipMergeFlagMinBlob:
    "Connected islands of this colour with fewer faces are absorbed by the surrounding colour. Recommended: skin 60, dark colours 120, others 150.",
  mergeUndoTitle: "Undo baked merge",
  mergeUndoAvailable: "baked merge step(s) can be undone",
  mergeUndoNone: "No baked merge to undo.",
  mergeUndo: "Undo",
  tipMergeUndo: "Restores the face colours from before the last baked merge / Nomad import.",
  mergeReportTitle: "Last merge",
  mergeReportGroup: "Colour",
  mergeReportBefore: "Before",
  mergeReportNaive: "Plain merge",
  mergeReportAfter: "After re-judgement",
  mergeReportColours: "Face colours",
  mergeReportAmbiguousFaces: "Re-judged faces",
  mergeReportPropagated: "Filled from neighbours",
  mergeReportRounds: "rounds",
  mergeReportFallback: "Kept user merge",
  mergeReportSmoothed: "Boundary fixes",
  mergeReportIslands: "Islands absorbed",
  viewFront: "Front",
  viewBack: "Back",
  progressMergeTitle: "Merge with shade re-judgement",
  progressMergeAdjacency: "Build face adjacency",
  progressMergeReproject: "Re-judge shade colours and clean up",
  progressMergeApply: "Apply new face colours",
  progressMergeCompare: "Render before/after comparison",
  mergeReprojectedDone: "Merged with shade re-judgement:",
  mergeUndone: "Last baked merge undone.",
  mergeNeedsModel: "Load a model before merging.",

  // --- Feature C (Nomad round trip) ---
  nomadTitle: "Nomad Sculpt round trip (optional)",
  nomadIntro:
    "After the colour count is final and before the virtual mixes matter: export the model with its N palette colours, touch up eyes and boundaries in Nomad, then load the OBJ back. Colours are snapped to the palette; the geometry is kept when the face order is unchanged.",
  nomadExportTitle: "Export vertex-colour OBJ for Nomad",
  nomadExportColours: "palette colours will be written (vertex colour = majority of adjacent faces, no vertex welding)",
  nomadExportUnavailable: "Load a model and build the palette first.",
  nomadExport: "Export OBJ",
  tipNomadExport:
    "Writes model_<N>colors_for_nomad.obj. Vertices are not merged by position so the face order can be matched when the file comes back.",
  nomadImportTitle: "Load the OBJ exported from Nomad",
  tipNomadImport:
    "Reads v x y z r g b vertex colours. Same vertex/face count: only colours are updated. Different counts (remeshed): the geometry is replaced after a warning. Every vertex colour is snapped to the nearest palette colour (CIEDE2000).",
  nomadRulesTitle: "Nomad usage rules",
  nomadExported: "Nomad OBJ exported:",
  nomadImported: "Nomad OBJ applied:",
  nomadColoursOnly: "colours updated, geometry kept",
  nomadFullReload: "geometry replaced",
  nomadOffPalette: "vertices off palette",
  nomadColoursAfter: "face colours",

  // --- Feature D (export) ---
  exportIncludePrinterConfig: "Include printer config from template (Slic3r_PE.config)",
  tipExportIncludePrinterConfig:
    "Off (recommended): no Slic3r_PE.config is written, so PrusaSlicer keeps the printer/filament profiles you selected (fixes XL wipe-tower errors caused by single_extruder_multi_material = 1). On: the config from the loaded 3MF template is copied with extended slot arrays.",
  exportPrinterConfigOmitted: "printer profile left to PrusaSlicer (no Slic3r_PE.config)",
  exportPrinterConfigIncluded: "template Slic3r_PE.config included",
  exportVirtualIds: "virtual extruder ids",
  ...
  physicalDirectSuggestionDescription:
    "Palette colours within ΔE00 < 6 of a physical filament (typically black and white) print cleaner as that physical extruder than as an alternating-layer mixture.",
  applyPhysicalDirectSuggestions: "Apply recommendations",
  tipApplyPhysicalDirectSuggestions:
    "Assigns every recommended palette colour directly to the matching physical extruder.",
  physicalDirectBadge: "physical",
  noPhysicalDirectSuggestions: "No recommendations for the current palette.",
  physicalDirectApplied: "Recommended physical assignments applied.",
  stripeNote:
    "Defaults: thirds recipes and 2 colours per mixture keep the layer period short (5% steps = 20-layer period = visible 1 mm stripes at 0.05 mm layers).",
};
