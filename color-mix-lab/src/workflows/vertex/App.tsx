...
import { medianCutPalette, nearestPaletteIndex } from "./core/quantize";
...
import { loadRodin3mf, type Rodin3mfLoadResult } from "./core/rodin3mf";
...
import {
  ...
  buildNomadObj,
  parseNomadObj,
} from "./core/nomadRoundTrip";
import { rasterToDataUrl, renderModelThumbnail } from "./core/modelThumbnail";
import { deltaE2000, rgbToLab } from "./core/prusaFdmMixer";
import {
  MergeReprojectionPanel,
  type MergeReport,
  type MergeReportRow,
  type MergeReportThumbnails,
  type PaletteMergeFlag,
} from "./ui/MergeReprojectionPanel";
import { NomadRoundTripPanel, type NomadPanelReport } from "./ui/NomadRoundTripPanel";
import {
  applyAssignmentOverridesToPlan,
  type AssignmentOverride as CoreAssignmentOverride,
} from "./core/assignmentOverrides";
...
type AssignmentOverride = CoreAssignmentOverride;
...
function normalizeMaxVirtualMixComponents(value: unknown, fallback: 2 | 3 = 2): 2 | 3 {
...
export default function App({
  ...
  // Rodin pipeline state (features A-D of the Rodin work order).
  const [pendingRodinFile, setPendingRodinFile] = useState<File | null>(null);
  const [rodinLoadInfo, setRodinLoadInfo] = useState<{
    fileName: string;
    paletteHex: string[];
    paletteSource: Rodin3mfLoadResult["paletteSource"];
    usedPaletteNumbers: number[];
    warnings: string[];
    stats: Rodin3mfLoadResult["stats"];
  } | null>(null);
  const [reprojectOnMerge, setReprojectOnMerge] = useState(true);
  const [paletteMergeFlags, setPaletteMergeFlags] = useState<
    Record<string, PaletteMergeFlag>
  >({});
  const [mergeHistory, setMergeHistory] = useState<
    Array<{ triangleColors: RGB[]; label: string }>
  >([]);
  const [lastMergeReport, setLastMergeReport] = useState<MergeReport | null>(null);
  const [mergeBusy, setMergeBusy] = useState(false);
  const [nomadBusy, setNomadBusy] = useState(false);
  const [nomadReport, setNomadReport] = useState<NomadPanelReport | null>(null);
  const [nomadFileKey, setNomadFileKey] = useState(0);
  const [exportIncludePrinterConfig, setExportIncludePrinterConfig] = useState(false);
  ...
  const [pendingRecipeResolution, setPendingRecipeResolution] =
    useState<MixingRecipeResolution>("thirds");
  ...
  const modelFileKindRef = useRef<"obj" | "rodin3mf">("obj");
  const faceAdjacencyRef = useRef<{ triangles: Tri[]; adjacency: FaceAdjacency } | null>(null);
  ...
  async function handleLoadSelectedInputs(): Promise<void> {
    const hasAnyPendingInput = Boolean(
      pendingObjFile || pendingRodinFile || pendingTemplateFile || pendingFilamentListFile,
    );
    ...
    try {
      ...
      if (pendingRodinFile) {
        ...
        await onRodin3mfFile(pendingRodinFile);
        setActiveTab("physical");
      } else if (pendingObjFile) {
        showProgress("load", t.progressLoadTitle, loadSteps, 3);
  ...
  async function onObjFile(
    ...
    faceAdjacencyRef.current = null;
    setRodinLoadInfo(null);
    setMergeHistory([]);
    setLastMergeReport(null);
    setPaletteMergeFlags({});
    setNomadReport(null);
  ...
  /**
   * Feature A: load a Hyper3D Rodin "3D Print" 3MF (Output Mode: Face Color).
   * The result is the same internal state as a baked vertex-colour OBJ, so the
   * Texture Baking workflow is skipped entirely.
   */
  async function onRodin3mfFile(
    file: File,
    options: { jumpToPhysical?: boolean } = {},
  ) {
    const jumpToPhysical = options.jumpToPhysical ?? true;
    setFileLoadBusy(true);
    setStatus(t.loadingRodin);
    setModel(null);
    baseModelRef.current = null;
    faceAdjacencyRef.current = null;
    orientationMatrixRef.current = [...IDENTITY_ORIENTATION_MATRIX];
    ...
    await new Promise((resolve) => window.setTimeout(resolve, 40));
    try {
      const safeModelName = safeFileDisplayName(file.name, "rodin_model.3mf");
      const result = await loadRodin3mf(file, safeModelName);
      modelFileRef.current = file;
      modelFileKindRef.current = "rodin3mf";
      baseModelRef.current = result.model;
      orientationMatrixRef.current = [...IDENTITY_ORIENTATION_MATRIX];
      setModel(modelWithOrientationMatrix(result.model, orientationMatrixRef.current));
      setExportFileName(exportFileNameForModel(safeModelName));
      exportFileNameUserEditedRef.current = false;
      // Rodin 3MF files are millimetre, Z-up with the base at z = 0: never
      // let the auto heuristic rotate them.
      setExportCoordinateMode("keep");
      setRodinLoadInfo({
        fileName: safeModelName,
      ...
      setStatus(
        `${t.rodinLoaded} ${result.stats.triangleCount.toLocaleString()} ${t.triangles}, ${result.stats.vertexCount.toLocaleString()} ${t.vertices}, ${result.stats.usedColourCount}/${result.stats.paletteCount} ${t.colours}`,
      );
      if (jumpToPhysical) setActiveTab("physical");
    } catch (err) {
      setStatus(
  ...
  async function buildProjectExport(): Promise<string> {
    ...
    // Only OBJ models are embedded; a Rodin 3MF (100 MB class) stays external.
    if (saveProjectParts.model && modelFileRef.current && modelFileKindRef.current === "obj") {
  ...
  // ---------------------------------------------------------------------------
  // Rodin pipeline: per-triangle palette labels, merge with shade re-judgement
  // (feature B), Nomad round trip (feature C) and physical-direct
  // recommendations (work order §6).
  // ---------------------------------------------------------------------------
  const mergeSteps = useMemo(
    () => [
      t.progressMergeAdjacency,
      t.progressMergeReproject,
      t.progressMergeApply,
      t.progressMergeCompare,
    ],
    [t],
  );

  /** Palette position (0-based) of every triangle, matching the export mapping. */
  const trianglePaletteLabels = useMemo(() => {
    const labels = new Int32Array(adjustedColors.length);
    if (palette.length === 0) return labels;
    const cache = new Map<number, number>();
    for (let i = 0; i < adjustedColors.length; i++) {
      const rgb = adjustedColors[i];
      const key = (rgb[0] << 16) | (rgb[1] << 8) | rgb[2];
      let label = cache.get(key);
      if (label === undefined) {
        label = nearestPaletteIndex(rgb, palette, appliedAccentProtection);
        cache.set(key, label);
      }
      labels[i] = label;
    }
    return labels;
  }, [adjustedColors, palette, appliedAccentProtection]);

  const paletteAreaFractionByIndex = useMemo(() => {
    const map = new Map<number, number>();
    if (palette.length === 0) return map;
    const weights =
      triangleAreaWeights.length === trianglePaletteLabels.length
        ? triangleAreaWeights
        : undefined;
    const fractions = groupWeightFractions(trianglePaletteLabels, palette.length, weights);
    palette.forEach((entry, position) => map.set(entry.index, fractions[position]));
    return map;
  }, [palette, trianglePaletteLabels, triangleAreaWeights]);

  // Suggest the shade / protect / min-island flags for new palette colours.
  useEffect(() => {
    if (palette.length === 0) return;
    setPaletteMergeFlags((prev) => {
      let changed = false;
      const next = { ...prev };
      for (const entry of palette) {
        const hex = rgbToHex(entry.rgb);
        if (next[hex]) continue;
        const area = paletteAreaFractionByIndex.get(entry.index) ?? 0;
        next[hex] = {
          ambiguous: isLikelyAmbiguousColour(entry.rgb, area),
          protect: isLikelyProtectedColour(entry.rgb, area),
          minBlob: defaultMinBlobForColour(entry.rgb),
        };
        changed = true;
      }
      return changed ? next : prev;
    });
  }, [palette, paletteAreaFractionByIndex]);

  function updatePaletteMergeFlag(hex: string, patch: Partial<PaletteMergeFlag>): void {
    setPaletteMergeFlags((prev) => ({
      ...prev,
      [hex]: {
        ...(prev[hex] ?? { ambiguous: false, protect: false, minBlob: DEFAULT_MIN_BLOB }),
        ...patch,
      },
    }));
  }

  const PHYSICAL_DIRECT_DELTA_E = 6;
  const physicalDirectSuggestions = useMemo(() => {
    const suggestions = new Map<number, { extruder: number; deltaE: number }>();
    if (currentPhysicalSlots.length === 0 || palette.length === 0) return suggestions;
    const paletteByIndex = new Map(palette.map((entry) => [entry.index, entry]));
    const slotLabs = currentPhysicalSlots.map((slot) => ({
      slot: slot.slot,
      lab: rgbToLab(slot.filament.effectiveRgb),
    }));
    for (const entry of virtualExtruderPlan.virtualBlends) {
      for (const paletteIndex of entry.targetPaletteIndices) {
        const paletteEntry = paletteByIndex.get(paletteIndex);
        if (!paletteEntry) continue;
        const lab = rgbToLab(paletteEntry.rgb);
        let best: { extruder: number; deltaE: number } | null = null;
        ...
        if (best && best.deltaE < PHYSICAL_DIRECT_DELTA_E) suggestions.set(paletteIndex, best);
      }
    }
    return suggestions;
  }, [virtualExtruderPlan, palette, currentPhysicalSlots]);

  function physicalDirectBadgeFor(paletteIndices: number[]): React.ReactNode {
    for (const paletteIndex of paletteIndices) {
      const suggestion = physicalDirectSuggestions.get(paletteIndex);
      if (!suggestion) continue;
      return (
        <span className="rodin-suggestion-badge" title={t.physicalDirectSuggestionDescription}>
          → E{suggestion.extruder} {t.physicalDirectBadge} (ΔE {suggestion.deltaE.toFixed(1)})
        </span>
      );
    }
    return null;
  ...
  function applyPhysicalDirectSuggestions(): void {
    if (physicalDirectSuggestions.size === 0) return;
    setAssignmentOverrides((prev) => {
      const next = { ...prev };
      for (const [paletteIndex, suggestion] of physicalDirectSuggestions)
        next[paletteIndex] = { kind: "physical", extruder: suggestion.extruder };
      return next;
    });
    setSelectedAssignmentKeys([]);
    void runVirtualRecalculationProgress(t.physicalDirectApplied);
  }

  function colourKeyOf(rgb: RGB): number {
    return (rgb[0] << 16) | (rgb[1] << 8) | rgb[2];
  }

  function countUniqueColours(colours: RGB[]): number {
    const keys = new Set<number>();
    for (const rgb of colours) keys.add(colourKeyOf(rgb));
    return keys.size;
  }

  function faceAdjacencyForBase(base: MeshModel): FaceAdjacency {
    const cached = faceAdjacencyRef.current;
    if (cached && cached.triangles === base.triangles) return cached.adjacency;
    const adjacency = buildFaceAdjacency(base.triangles, base.vertices.length);
    faceAdjacencyRef.current = { triangles: base.triangles, adjacency };
    return adjacency;
  }

  /**
   * Most frequent *unadjusted* model colour per palette position.  Baked
   * merges write these back into the model so that the colour correction is
   * not applied twice.
   */
  function representativeBaseColoursByPosition(base: MeshModel): RGB[] {
    const counts: Array<Map<number, number>> = palette.map(() => new Map());
    const labels = trianglePaletteLabels;
    for (let i = 0; i < labels.length; i++) {
      const rgb = base.triangleColors[i];
      const bucket = counts[labels[i]];
      if (!rgb || !bucket) continue;
      const key = colourKeyOf(rgb);
    ...
    return palette.map((entry, position) => {
      ...
      if (bestKey < 0) return entry.rgb;
      return [(bestKey >> 16) & 255, (bestKey >> 8) & 255, bestKey & 255];
    });
  }

  function adjustedFromBase(colours: RGB[]): RGB[] {
    if (isDefaultAdjustments(appliedAdjustments)) return colours;
    const cache = new Map<number, RGB>();
    return colours.map((rgb) => {
      const key = colourKeyOf(rgb);
      let out = cache.get(key);
      if (!out) {
        out = adjustColour(rgb, appliedAdjustments);
        cache.set(key, out);
      }
      return out;
    });
  ...
  async function renderCompareThumbnails(
    coloursBefore: RGB[],
    coloursAfter: RGB[],
  ): Promise<MergeReportThumbnails | null> {
    if (!model) return null;
    try {
      const render = (colours: RGB[], view: "front" | "back") =>
        rasterToDataUrl(
          renderModelThumbnail(model, (i) => colours[i] ?? [180, 180, 180], {
            width: 260,
            height: 260,
            view,
          }),
        );
      const [beforeFront, beforeBack, afterFront, afterBack] = await Promise.all([
        render(coloursBefore, "front"),
        render(coloursBefore, "back"),
        render(coloursAfter, "front"),
        render(coloursAfter, "back"),
      ]);
      return { beforeFront, beforeBack, afterFront, afterBack };
    } catch {
      return null;
    }
  }

  /** Replace the model face colours and keep the previous ones for Undo. */
  function applyBakedColours(nextColours: RGB[], historyLabel: string): void {
    const base = baseModelRef.current;
    if (!base) return;
    setMergeHistory((prev) => [
      ...prev.slice(-9),
      { triangleColors: base.triangleColors, label: historyLabel },
    ]);
    const nextBase: MeshModel = {
      ...base,
      triangleColors: nextColours,
      stats: { ...base.stats, uniqueFaceColors: countUniqueColours(nextColours) },
    };
    baseModelRef.current = nextBase;
    setModel(modelWithOrientationMatrix(nextBase, orientationMatrixRef.current));
    setAssignmentOverrides({});
    setSelectedAssignmentKeys([]);
  }

  /** Feature B: merge the selected palette colours with shade re-judgement. */
  async function runReprojectedMerge(selected: number[]): Promise<void> {
    const base = baseModelRef.current;
    if (!base || !model || palette.length === 0 || selected.length < 2) {
      setStatus(t.mergeNeedsModel);
      return;
    }
    if (trianglePaletteLabels.length !== base.triangleColors.length) return;
    setMergeBusy(true);
    showProgress("palette", t.progressMergeTitle, mergeSteps, 0);
    await yieldToUi(40);
    try {
      const positionByIndex = new Map(palette.map((entry, position) => [entry.index, position]));
      const targetIndex = selected[0];
      const targetPosition = positionByIndex.get(targetIndex);
      if (targetPosition === undefined) throw new Error("Merge target is not in the palette.");
      const selectedPositions = new Set(
        selected
          .map((index) => positionByIndex.get(index))
          .filter((position): position is number => position !== undefined),
      );

      // Every palette colour keeps its own group except the selected ones,
      // which join the group of the first selected colour.
      const labelToGroup = new Int32Array(palette.length);
      const groupHeadPosition: number[] = [];
      let targetGroup = -1;
      palette.forEach((_entry, position) => {
        if (selectedPositions.has(position) && position !== targetPosition) return;
        labelToGroup[position] = groupHeadPosition.length;
        if (position === targetPosition) targetGroup = groupHeadPosition.length;
        groupHeadPosition.push(position);
      });
      for (const position of selectedPositions) {
        if (position !== targetPosition) labelToGroup[position] = targetGroup;
      }
      const groupCount = groupHeadPosition.length;

      const flagOf = (position: number) => paletteMergeFlags[rgbToHex(palette[position].rgb)];
      const ambiguousLabels: number[] = [];
      for (const position of selectedPositions) {
        if (position !== targetPosition && flagOf(position)?.ambiguous) ambiguousLabels.push(position);
      }
      const protectGroups = new Set<number>();
      palette.forEach((_entry, position) => {
        if (flagOf(position)?.protect) protectGroups.add(labelToGroup[position]);
      });
      const minBlob = new Map<number, number>();
      groupHeadPosition.forEach((head, group) =>
        minBlob.set(group, flagOf(head)?.minBlob ?? DEFAULT_MIN_BLOB),
      );

      const adjacency = faceAdjacencyForBase(base);
      showProgress("palette", t.progressMergeTitle, mergeSteps, 1);
      await yieldToUi(20);
      const result = mergeWithReprojection(adjacency, {
        faceLabels: trianglePaletteLabels,
        labelToGroup,
        groupCount,
        ambiguousLabels,
        protectGroups,
      ...
      showProgress("palette", t.progressMergeTitle, mergeSteps, 2);
      await yieldToUi(20);
      const representatives = representativeBaseColoursByPosition(base);
      const groupColour = groupHeadPosition.map((head) => representatives[head]);
      const nextColours: RGB[] = new Array(result.groups.length);
      for (let i = 0; i < result.groups.length; i++) nextColours[i] = groupColour[result.groups[i]];

      const weights =
        triangleAreaWeights.length === result.groups.length ? triangleAreaWeights : undefined;
      const beforeByPosition = groupWeightFractions(trianglePaletteLabels, palette.length, weights);
      const naive = groupWeightFractions(result.naiveGroups, groupCount, weights);
      const after = groupWeightFractions(result.groups, groupCount, weights);
      const rows: MergeReportRow[] = groupHeadPosition
        .map((head, group) => ({
          label: `#${palette[head].index} ${rgbToHex(palette[head].rgb)}`,
          rgb: palette[head].rgb,
          before: beforeByPosition[head],
          naive: naive[group],
          after: after[group],
          merged: group === targetGroup,
        }))
        .sort((a, b) => Number(b.merged) - Number(a.merged) || b.after - a.after);

      showProgress("palette", t.progressMergeTitle, mergeSteps, 3);
      await yieldToUi(20);
      const thumbnails = await renderCompareThumbnails(
        adjustedColors,
        adjustedFromBase(nextColours),
      );

      const selectedLabel = selected.map((index) => `#${index}`).join(" + ");
      applyBakedColours(nextColours, `merge ${selectedLabel}`);
      setLastMergeReport({
        title: `${t.mergeReportTitle}: ${selectedLabel} → #${targetIndex}`,
        rows,
        stats: result.stats,
        thumbnails,
        faceColourCountBefore: countUniqueColours(base.triangleColors),
        faceColourCountAfter: countUniqueColours(nextColours),
      });
      setStatus(
        `${t.mergeReprojectedDone} ${selected.length} → 1 (${t.mergeReportAmbiguousFaces}: ${result.stats.ambiguousFaceCount.toLocaleString()}, ${t.mergeReportSmoothed}: ${result.stats.smoothedFaceChanges.toLocaleString()}, ${t.mergeReportIslands}: ${result.stats.absorbedIslands})`,
      );
      showProgress("palette", t.progressMergeTitle, mergeSteps, mergeSteps.length - 1);
    } catch (err) {
      showProgress(
        "palette",
        t.progressMergeTitle,
        mergeSteps,
        0,
        err instanceof Error ? err.message : String(err),
      );
  ...
  function undoLastMerge(): void {
    const base = baseModelRef.current;
    const last = mergeHistory[mergeHistory.length - 1];
    if (!base || !last) return;
    if (last.triangleColors.length !== base.triangles.length) {
      setMergeHistory([]);
      return;
    }
    const restored: MeshModel = {
      ...base,
      triangleColors: last.triangleColors,
      stats: { ...base.stats, uniqueFaceColors: countUniqueColours(last.triangleColors) },
    };
    baseModelRef.current = restored;
    setModel(modelWithOrientationMatrix(restored, orientationMatrixRef.current));
    setMergeHistory((prev) => prev.slice(0, -1));
    setLastMergeReport(null);
    setAssignmentOverrides({});
    setSelectedAssignmentKeys([]);
    setStatus(t.mergeUndone);
  }

  /** Feature C-1: vertex-colour OBJ for Nomad with exactly the palette colours. */
  function handleNomadExport(): void {
    if (!model || palette.length === 0) return;
    setNomadBusy(true);
    try {
      const paletteRgb = palette.map((entry) => entry.rgb);
      const result = buildNomadObj(model, adjustedColors, paletteRgb, model.name);
      downloadText(result.fileName, result.obj);
      setNomadReport({
        message: `${t.nomadExported} ${result.fileName} · ${result.colourCount} ${t.colours} · ${result.vertexCount.toLocaleString()} ${t.vertices} · ${result.faceCount.toLocaleString()} ${t.triangles}`,
        warnings: [],
      });
      setStatus(`${t.nomadExported} ${result.fileName}`);
    } catch (err) {
      setStatus(`${t.error}: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
      setNomadBusy(false);
  ...
  /** Feature C-2: apply the OBJ that came back from Nomad. */
  async function onNomadObjFile(file: File): Promise<void> {
    setNomadBusy(true);
    setStatus(t.loadingFile);
    try {
      const base = baseModelRef.current;
      const text = await file.text();
      const parsed = parseNomadObj(text);
      const paletteRgb = palette.map((entry) => entry.rgb);
      const name = safeFileDisplayName(file.name, "nomad.obj");
      const result = applyNomadObjToModel(base, parsed, paletteRgb, name);
      if (result.mode === "colours-only" && base) {
        // Map the snapped palette colours back to unadjusted model colours.
        const representatives = representativeBaseColoursByPosition(base);
        const positionByKey = new Map(paletteRgb.map((rgb, position) => [colourKeyOf(rgb), position]));
        const nextColours = result.model.triangleColors.map((rgb) => {
          const position = positionByKey.get(colourKeyOf(rgb));
          return position === undefined ? rgb : representatives[position];
        });
        applyBakedColours(nextColours, `nomad ${name}`);
      } else {
        // Remeshed or no model: adopt the Nomad geometry as a new model.
        modelFileRef.current = file;
        modelFileKindRef.current = "obj";
        baseModelRef.current = result.model;
        faceAdjacencyRef.current = null;
        ...
        setExportFileName(exportFileNameForModel(result.model.name));
        exportFileNameUserEditedRef.current = false;
        setExportCoordinateMode("keep");
      }
      const summary = `${t.nomadImported} ${result.mode === "colours-only" ? t.nomadColoursOnly : t.nomadFullReload} · ${result.colourCount} ${t.nomadColoursAfter} · ${result.snap.offPaletteVertexCount.toLocaleString()} ${t.nomadOffPalette} (max ΔE ${result.snap.maxDeltaE.toFixed(1)})`;
      setNomadReport({ message: summary, warnings: result.warnings });
      setStatus(result.warnings.length > 0 ? `${summary} · ${result.warnings[0]}` : summary);
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      setNomadReport({ message: `${t.error}: ${message}`, warnings: [] });
      setStatus(`${t.error}: ${message}`);
    } finally {
      setNomadBusy(false);
      setNomadFileKey((value) => value + 1);
    }
  }
  ...
  async function handleReloadData(): Promise<void> {
    ...
    try {
      ...
      showProgress("load", t.progressLoadTitle, loadSteps, 3);
      if (modelFileKindRef.current === "rodin3mf") await onRodin3mfFile(objFile);
      else await onObjFile(objFile);
      showProgress("load", t.progressLoadTitle, loadSteps, 4);
      await yieldToUi(80);
    } catch (err) {
      showProgress(
  ...
  function mergeSelectedVirtualAssignments(): void {
    ...
    if (reprojectOnMerge && baseModelRef.current) {
      // Feature B: bake the merge into the model with shade re-judgement.
      void runReprojectedMerge(selectedPaletteIndices);
      return;
    }
  ...
  async function handleExport3mf() {
    ...
    try {
      ...
      const result = await buildPrusa3mfBlob({
        ...
        accentProtection: appliedAccentProtection,
        includePrinterConfig: exportIncludePrinterConfig,
        effectiveRgbByPaletteIndex: effectivePaletteRgbByIndex,
      });
      ...
      const renumbered =
        result.summary.renumberedVirtuals.length > 0
          ? `, ${t.exportRenumbered}: ${result.summary.renumberedVirtuals
              .map((item) => `VE${item.from}→VE${item.to}`)
              .join(" ")}`
          : "";
      setStatus(
        `${t.export3mfDone} ${result.summary.virtualCount} ${t.virtualExtrudersShort} (${t.exportVirtualIds}: ${result.summary.virtualIds.join(", ") || "-"}${renumbered}), ${result.summary.printerConfigIncluded ? t.exportPrinterConfigIncluded : t.exportPrinterConfigOmitted}, ${t.coordinateMode}: ${result.summary.coordinateMode}, ${t.scale}: ${result.summary.scale.toFixed(4)}`,
      );
    } catch (err) {
      showProgress(
        "export",
          ...
          <div className="tab-content">
              ...
              <>
                <section className="card load-card staged-load-card">
                  ...
                  <div className="staged-file-grid">
                    ...
                    <label
                      className="file-drop staged-file-picker"
                      title={t.tipRodin3mfFile}
                    >
                      <span>{t.rodin3mfFile}</span>
                      <strong>
                        {pendingRodinFile
                          ? fileSummary(pendingRodinFile)
                          : t.chooseFile}
                      </strong>
                      <input
                        disabled={fileLoadBusy}
                        type="file"
                        accept=".3mf"
                        key={`rodin-${filePickerResetKey}`}
                        onChange={(e) =>
                          setPendingRodinFile(e.target.files?.[0] ?? null)
                        }
                      />
                    </label>
                    <label
                  ...
                  {rodinLoadInfo && (
                    <div className="file-summary full rodin-summary">
                      <b>
                        {t.rodinSummaryTitle}: {rodinLoadInfo.fileName}
                      </b>
                      <span>
                        {t.rodinPalette}: {rodinLoadInfo.stats.paletteCount} ·{" "}
                        {rodinLoadInfo.stats.usedColourCount} {t.rodinUsedColours}
                        {rodinLoadInfo.stats.unpaintedTriangleCount > 0
                          ? ` · ${rodinLoadInfo.stats.unpaintedTriangleCount.toLocaleString()} ${t.rodinUnpainted}`
                          : ""}
                        {rodinLoadInfo.paletteSource === "fallback"
                          ? ` · ${t.rodinPaletteFallback}`
                          : ""}
                      </span>
                      <span className="rodin-palette-row">
                        {rodinLoadInfo.paletteHex.map((hex, index) => {
                          const used = rodinLoadInfo.usedPaletteNumbers.includes(index + 1);
                          return (
                            <span
                              key={`${hex}-${index}`}
                              className={`swatch${used ? "" : " unused"}`}
                              style={{ background: hex }}
                              title={`E${index + 1} ${hex}${used ? "" : " (unused)"}`}
                            />
                          );
                        })}
                      </span>
                      {rodinLoadInfo.warnings.map((warning) => (
                        <span key={warning} className="warning-text">
                          {warning}
                        </span>
                      ))}
                      <span className="muted">{t.rodinTextureModeHint}</span>
                    </div>
                  )}
                ...
                >
                  ...
                  {palette.length > 0 && currentPhysicalSlots.length > 0 ? (
                    <>
                      ...
                      >
                        ...
                        <div className="virtual-edit-actions stacked">
                          ...
                          <label
                            ...
                            title={t.tipMergeReprojectToggle}
                          >
                            <input
                              type="checkbox"
                              checked={reprojectOnMerge}
                              disabled={virtualEditBusy || mergeBusy}
                              onChange={(e) =>
                                setReprojectOnMerge(e.target.checked)
                              }
                            />
                            {t.mergeReprojectToggle}
                          </label>
                          <div className="virtual-edit-action-row">
                            <div>
                      ...
                      <div className="virtual-edit-box">
                        <div className="virtual-edit-action-row">
                          <div>
                            <b>{t.physicalDirectSuggestionTitle}</b>
                            <p className="muted small-note">
                              {physicalDirectSuggestions.size > 0
                                ? [...physicalDirectSuggestions.entries()]
                                    .map(
                                      ([paletteIndex, suggestion]) =>
                                        `#${paletteIndex} → E${suggestion.extruder} (ΔE ${suggestion.deltaE.toFixed(1)})`,
                                    )
                                    .join(", ")
                                : t.noPhysicalDirectSuggestions}
                            </p>
                            <p className="muted small-note">
                              {t.physicalDirectSuggestionDescription} {t.stripeNote}
                            </p>
                          </div>
                          <button
                            type="button"
                            ...
                            onClick={applyPhysicalDirectSuggestions}
                            title={t.tipApplyPhysicalDirectSuggestions}
                          >
                            {t.applyPhysicalDirectSuggestions}
                          </button>
                        </div>
                      </div>
                      ...
                      <MergeReprojectionPanel
                        t={t}
                        palette={palette}
                        areaFractionByIndex={paletteAreaFractionByIndex}
                        flags={paletteMergeFlags}
                        onFlagChange={updatePaletteMergeFlag}
                        report={lastMergeReport}
                        undoCount={mergeHistory.length}
                        onUndo={undoLastMerge}
                        busy={mergeBusy || virtualEditBusy}
                      />
                            ...
                            {filteredVirtualBlends.map((entry) => (
                              ...
                              >
                                ...
                                >
                                  ...
                                  {physicalDirectBadgeFor(entry.targetPaletteIndices)}
                                </small>
                              </div>
                            ))}
                          </div>
            ...
            {activeTab === "export" && (
              <section className="card tab-card export-card">
                ...
                <label
                  className="inline-check"
                  title={t.tipExportIncludePrinterConfig}
                >
                  <input
                    type="checkbox"
                    checked={exportIncludePrinterConfig && Boolean(templateInfo?.configFound)}
                    disabled={!templateInfo?.configFound}
                    onChange={(e) =>
                      setExportIncludePrinterConfig(e.target.checked)
                    }
                  />
                  {t.exportIncludePrinterConfig}
                </label>
                <p className="muted small-note">
                  {exportIncludePrinterConfig && templateInfo?.configFound
                    ? t.exportPrinterConfigIncluded
                    : t.exportPrinterConfigOmitted}
                  {" · "}
                  {t.exportValidationText}
                </p>
                ...
                <NomadRoundTripPanel
                  ...
                  onExport={handleNomadExport}
                  onImportFile={(file) => void onNomadObjFile(file)}
                  busy={nomadBusy || fileLoadBusy || mergeBusy}
                  report={nomadReport}
                  fileInputKey={nomadFileKey}
                />
