...
- **템플릿 기억**: 불러온 PrusaSlicer 템플릿을 브라우저(IndexedDB)에 저장해 다음 실행 때 자동으로 불러옵니다. "이 템플릿을 브라우저에 기억" 체크로 끄고 켭니다.
- **필라멘트 목록**: `이름; 종류; #색상` 형식(Filament-DB 내보내기, 예: `Prusament PLA Jet Black; PLA; #24292A`)의 .txt/.csv를 넣거나 붙여넣으면 슬롯마다 "목록에서…"로 고를 수 있고, "목록에서 자동 추천"이 모델 팔레트에 맞는 필라멘트를 채웁니다. 색은 색상 선택기나 hex 입력으로 직접 넣을 수도 있습니다.
- **색 영역 표시**: 병합 표의 행에 마우스를 올리면 미리보기에서 그 색의 면들이 파란 윤곽으로 빛나고, 선택(체크)한 색은 노란 윤곽으로 표시됩니다. 비슷한 색이 어디에 있는지 바로 확인할 수 있습니다.
- **묶음 클릭 → 색 바꾸기**: 미리보기에서 색 묶음(같은 색으로 이어진 영역)을 클릭하면 주황 윤곽으로 선택되고, 팔레트의 다른 색이나 새 색으로 그 묶음만 바꿀 수 있습니다(되돌리기 가능). 흰색이 두 군데 있을 때 한 군데만 빨강으로 바꾸는 식입니다.
- **다중 선택 병합**: 행 앞의 체크박스로 여러 색을 고르면 "선택한 N색 → [대상] 선택 병합 실행" 바가 나타납니다. 그늘/경계·보호는 좌우 토글 스위치입니다.
